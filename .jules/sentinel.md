...
### Performance Optimization: AI Batch Categorization
**Issue:** Found an N+1 query pattern where the system was making parallel, individual API calls to an LLM service for each uncategorized transaction using `Promise.all`. This architecture leads to significant network latency and high risks of hitting provider rate limits.
**Solution:** Consolidated individual transaction inputs into a single batch prompt asking for a JSON array output.
**Impact:** Simulated benchmarks demonstrated an ~85% latency reduction by cutting the API overhead from N network calls to just 1 call for standard batch sizes, vastly improving processing speeds and reducing rate-limiting vulnerabilities.
